...
- Compare the performance of this filter with :
  ...
  - the $\ell_{q}$-regularization
  - the mixed-norm $\ell_{p,q}$-regularization
"""
...
begin
	...
	pos_m = findall(ωfem/2π .≤ 2freqH[end]) # Indices of the modes kept in the basis
	ωₙ = ωfem[pos_m]
...
begin
	...
	CC_bf = round(CorrCoeff(Fᵣ[:], abs.(u_bf[:])), digits = 2)
	GREx_bf = round(RE(yᵣ[:], x_bf[:]), digits = 2)
	REx_bf = round(RE(yᵣ[Nₒ, :], x_bf[Nₒ, :]), digits = 2)
	CCx_bf = round(CorrCoeff(abs.(yᵣ[:]), abs.(x_bf[:])), digits = 2)
...
begin
	...
	CC_bfr = round(CorrCoeff(Fᵣ[:], abs.(u_bfr[:])), digits = 2)
	GREx_bfr = round(RE(yᵣ[:], x_bfr[:]), digits = 2)
	REx_bfr = round(RE(yᵣ[Nₒ, : ], x_bfr[Nₒ, :]), digits = 2)
	CCx_bfr = round(CorrCoeff(abs.(yᵣ[:]), abs.(x_bfr[:])), digits = 2)
...
begin
	...
	CC_rvr = round(CorrCoeff(Fᵣ[:], abs.(u_rvr[:])), digits = 2)
	GREx_rvr = round(RE(yᵣ[:], x_rvr[:]), digits = 2)
	REx_rvr = round(RE(yᵣ[Nₒ, :], x_rvr[Nₒ, :]), digits = 2)
	CCx_rvr = round(CorrCoeff(abs.(yᵣ[:]), abs.(x_rvr[:])), digits = 2)
...
begin
	...
	CC_lq = round(CorrCoeff(Fᵣ[:], abs.(u_lq[:])), digits = 2)
	GREx_lq = round(RE(yᵣ[:], x_lq[:]), digits = 2)
	REx_lq = round(RE(yᵣ[Nₒ, :], x_lq[Nₒ, :]), digits = 2)
	CCx_lq = round(CorrCoeff(abs.(yᵣ[:]), abs.(x_lq[:])), digits = 2)
...
begin
	...
	CC_lpq = round(CorrCoeff(Fᵣ[:], abs.(u_lpq[:])), digits = 2)
	GREx_lpq = round(RE(yᵣ[:], x_lpq[:]), digits = 2)
	REx_lpq = round(RE(yᵣ[Nₒ, :], x_lpq[Nₒ, :]), digits = 2)
	CCx_lpq = round(CorrCoeff(abs.(yᵣ[:]), abs.(x_lpq[:])), digits = 2)
