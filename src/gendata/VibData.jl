...
"""
    resp(ωₙ, ξₙ, ϕₘ, ϕₑ, freq, F)

Compute the response of a simply supported beam `b` for given natural frequencies `ωₙ`, damping ratios `ξₙ`, eigenmodes `ϕₒ` and `ϕₑ`, frequency range `freq` and excitation `F`.
...
# Output
- `y`: response - Matrix{Complex{Float64}} (dimension: length(ϕₘ) x length(freq))

# Example
```julia-repl
julia> yᵣ = resp(ωₙ, ξₙ, ϕₘ, ϕₑ, freq, F)
```
"""
function resp(ωₙ, ξₙ, ϕₘ, ϕₑ, freq, F)
    # Initialisation
    Nmodes = length(ωₙ)
    Nₘ = size(ϕₘ, 1)
    Nf = length(freq)

    # Initialisation des variables temporaires pour réduire l'allocation mémoire
    c = Vector{ComplexF64}(undef, Nₘ)
    d = Matrix{ComplexF64}(undef, Nₘ, Nmodes)
    e = Vector{Float64}(undef, Nmodes)
    M = Diagonal{ComplexF64}(undef, Nmodes)
    indm = diagind(M)

    ωf = 2π*freq
    y = Matrix{ComplexF64}(undef, (Nₘ, Nf))
    @inbounds for (f, (ω, Fₑ)) in enumerate(zip(ωf, F))
        @. M[indm] = -ω^2/(ωₙ^2 - ω^2 + 2im*ξₙ*ωₙ*ω)
        y[:, f] .= mul!(c, mul!(d, ϕₘ, M), mul!(e, ϕₑ', Fₑ))
    end
...
"""
    frf_modal(ωₙ, ξₙ, ϕₑ, freq)

Computes the modal frequency response function (FRF) of a simply supported beam `b` for given natural frequencies `ωₙ`, damping ratios `ξₙ`, eigenmodes `ϕₑ`, and frequency range `freq`.
...
# Output
- `H`: modal frequency response function - Array{Matrix{Complex{Float64}}} (dimension: length(freq) x length(ωₙ) x length(ϕₑ))

"""
function frf_modal(ωₙ, ξₙ, freq, ϕₑ = I(length(ωₙ)))
    ωf = 2π*freq
    ...
    c = Matrix{ComplexF64}(undef, Nmodes, Nₑ)
    FRF = [Matrix{ComplexF64}(undef, Nmodes, Nₑ) for _ in 1:Nf]
    @inbounds for (f, ω) in enumerate(ωf)
        @. M[indm] = -ω^2/(ωₙ^2 - ω^2 + 2im*ξₙ*ωₙ*ω)
        FRF[f] .= mul!(c, M, ϕₑ')
    end
