...
function noisevar1D(x)
    if !isreal(x)
        return noisevar1D(real(x)) + noisevar1D(imag(x))
    end
...
function noisevar1D_(x)
    ...
    hMin = 1e-6
    hMax = 0.99
    lb = (((1 + √(1+8*hMax^(2/N)))/4/hMax^(2/N))^2 - 1)/16;
    ub = (((1 + √(1+8*hMin^(2/N)))/4/hMin^(2/N))^2 - 1)/16;

    res = optimize(L -> func(L, z, s²), log10(lb), log10(ub))
